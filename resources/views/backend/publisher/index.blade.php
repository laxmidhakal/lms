...
@section('style')
<style>
  form.publisherform label.error {
    color: #dc3545;
    font-weight: normal;
  }
</style>
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-bordered table-hover">
            ...
            @endphp             
            <tr data-toggle="tooltip" data-placement="top" title="{{ $publisher->is_active == '1' ? 'This data is published':' This data is not published'}}"   style="background-color: {{$color}}">
              <td>{{$key+1}}</td>
              <td>
                <a href="{{URL::to('/')}}/home/publisher/{{$publisher->slug}}/detail" class="text-dark font-weight-bold">
                  {{$publisher->name}}
                </a>
              </td>
              <td>{{$publisher->address}}</td>
              <td class="text-center">
                <a href="{{URL::to('/')}}/home/publisher/isactive/{{$publisher->id}}">
                  <i class="fa {{ $publisher->is_active == '1' ? 'fa-check check-css':'fa-times cross-css cross-css'}}"></i>
                </a>
              </td>
              <td>
                <p id="publisher{{$publisher->id}}" ids="{{$publisher->id}}" class="text-center sort mb-0" contenteditable="plaintext-only">{{$publisher->sort_id}}</p>
              </td>
              <td class="text-center">
               <a href="{{URL::to('/')}}/home/publisher/{{$publisher->id}}/edit"><ion-icon size="small" name="md-create"></ion-icon></a> ||<a class="button delete-confirm" href="{{URL::to('/')}}/home/publisher/{{$publisher->id}}/delete"><ion-icon size="small" name="md-trash" color="danger"></ion-icon></a> 
              </td> 
            </tr>
            @endforeach
          </table>
...
<div class="modal fade" id="modal-default" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form role="form" method="POST" action="{{URL::to('/')}}/home/publisher/store" class="publisherform" id="signup" >
        {{ csrf_field() }}
...
<script type="text/javascript">
  $(".sort").keydown(function (e) {
    ...
    if (e.which == 9){
      ...
      value = document.getElementById('publisher'+id).innerHTML; //value of the text input
      var url= "{{URL::to('/')}}/home/sort/publisher/"+value;
      $.ajax({
        type:"POST",
        dataType:"JSON",
        url:url,
