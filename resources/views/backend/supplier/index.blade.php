      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table table-striped table-bordered table-hover">
            ...
            @endphp              
            <tr data-toggle="tooltip" data-placement="top" title="{{ $supplier->is_active == '1' ? 'This data is published':' This data is not published'}}"   style="background-color: {{$color}}">
              <td>{{$key+1}}</td>
              <td>{{$supplier->name}}</td>
              <td>{{$supplier->address}}</td>
              <td class="text-center">
                <a href="{{URL::to('/')}}/home/supplier/isactive/{{$supplier->id}}">
                  <i class="fa {{ $supplier->is_active == '1' ? 'fa-check check-css':'fa-times cross-css'}}"></i>
                </a>
              </td>
              <td>
                <p id="supplier{{$supplier->id}}" ids="{{$supplier->id}}" class="text-center sort mb-0" contenteditable="plaintext-only">{{$supplier->sort_id}}</p>
              </td>
              <td class="text-center">
               <a href="{{URL::to('/')}}/home/supplier/{{$supplier->id}}/edit"><ion-icon size="small" name="md-create"></ion-icon></a> ||<a class="button delete-confirm" href="{{URL::to('/')}}/home/supplier/{{$supplier->id}}/delete"><ion-icon size="small" name="md-trash" color="danger"></ion-icon></a> 
              </td> 
            </tr>
            @endforeach
          </table>
...
<div class="modal fade" id="modal-default" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form role="form" method="POST" action="{{URL::to('/')}}/home/supplier/store" class="supplierform" id="signup" >
        {{ csrf_field() }}
...
<script type="text/javascript">
  $(".sort").keydown(function (e) {
    ...
    if (e.which == 9){
      ...
      value = document.getElementById('supplier'+id).innerHTML; //value of the text input
      var url= "{{URL::to('/')}}/home/sort/supplier/"+value;
      $.ajax({
        type:"POST",
        dataType:"JSON",
        url:url,
